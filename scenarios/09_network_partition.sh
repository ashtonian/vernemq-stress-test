...
partition_nodes() {
    ...
    for a_idx in $a_indices; do
        for b_idx in $b_indices; do
            ...
            ssh_vmq "$a_idx" "sudo iptables -A INPUT -s ${b_ip} -j DROP && \
                              sudo iptables -A OUTPUT -d ${b_ip} -j DROP" || true
        done
    done
    ...
    for b_idx in $b_indices; do
        for a_idx in $a_indices; do
            ...
            ssh_vmq "$b_idx" "sudo iptables -A INPUT -s ${a_ip} -j DROP && \
                              sudo iptables -A OUTPUT -d ${a_ip} -j DROP" || true
        done
    done
}

heal_partition() {
    # Flush iptables rules on all nodes to restore connectivity.
    local total
    total=$(vmq_node_count)

    log_info "Healing partition: flushing iptables on all $total nodes"
    for (( i=0; i<total; i++ )); do
        ssh_vmq "$i" "sudo iptables -F" || true
    done
}
...
count_messages_subset() {
    ...
    for idx in "${indices[@]}"; do
        local count
        count=$(ssh_vmq "$idx" "$VMQ_ADMIN metrics show" 2>/dev/null \
