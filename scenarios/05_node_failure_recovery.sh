...
set_connection_count() {
    ...
    for i in "${!nodes[@]}"; do
        ssh_vmq "$i" "$VMQ_ADMIN set outgoing_clustering_connection_count=$count" || true
    done
}
...
count_qos1_delivered() {
    ...
    for i in "${!nodes[@]}"; do
        ...
        count=$(ssh_vmq "$i" "$VMQ_ADMIN metrics show" 2>/dev/null \
            | grep "mqtt_puback_sent " | awk '{print $NF}' || echo 0)
...
run_failure_phases() {
    ...
    wait
    sleep "$SETTLE"
    collect_metrics "${tag_prefix}_p5_killed"
    ...
    echo "${tag_prefix}_multi_failure_seconds,$multi_time" \
        >> "${RESULTS_DIR}/${SCENARIO_TAG}/timing.csv"
    sleep "$SETTLE"
    collect_metrics "${tag_prefix}_p5_recovered"
